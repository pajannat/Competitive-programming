...
class Solver:
    ...
    def __init__(self, N: int, source_pos: List[Pos], house_pos: List[Pos], C: int, house_joint_indices: List, MST_min_source: Pos):
        self.N = N
        ...
        while move_continue_flg:
            if move_trans_flg:
                ...
                if len(right_left_list) == 0:
                    ...
                    now_x += right_left_list.pop()
                    now_destruct_cnt = self.destruct(now_y, now_x)
                    destructed_field_list.append([now_y, now_x])


            # 岩盤の硬いほうに進んでいる場合はタテヨコ移動を切り替え
            if now_destruct_cnt > bef_destruct_cnt:
...
def main():
    ...
    solver = Solver(N, source_pos, house_pos, C, house_joint_indices, MST_min_source)
    solver.solve()
