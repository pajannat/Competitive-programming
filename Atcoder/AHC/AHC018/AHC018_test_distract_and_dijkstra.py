...
def dijkstra(matrix_list, start, terminal):
    ...
    # for node in path:
    #     if node != terminal:
    #         print(f'{str(node)} -> ', file=sys.stderr)
    #     else:
    #         print(f"{str(node)}", file=sys.stderr)     
    # print(f"-----距離-----", file=sys.stderr)
    # print(f"{distance[terminal]}", file=sys.stderr)
    return distance[terminal], path


def pos2Gidx(pos):
    Gidx = 20 * (pos.y//10) + 1 * (pos.x//10)
    return Gidx
...
def main():
    ...
    test_destruct_grid = []
    test_destruct_grid_broken = [[0] * 20 for _ in range(20)]
    ...
    for pos1 in test_destruct_grid:
        for pos2 in test_destruct_grid:
                ...
                if abs(pos1.y - pos2.y) + abs(pos1.x - pos2.x) > 10:
                    continue

                # pos1, pos2 の岩盤が破壊済みか否か
                pos1_broken_flg = False
                pos2_broken_flg = False
                if test_destruct_grid_broken[pos1.y//10][pos1.x//10]:
                    pos1_broken_flg = True
                if test_destruct_grid_broken[pos2.y//10][pos2.x//10]:
    ...
    for house_broken_pos_i in house_broken_pos:
        ...
        for source_broken_pos_i in source_broken_pos:
            ...
            # 経路長と経路を記録
            tmp_dist, tmp_path = dijkstra(G, pos2Gidx(h_broken_pos), pos2Gidx(s_broken_pos))
            # 経路長が最短のものを採用
